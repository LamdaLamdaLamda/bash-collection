...
sysctl -w net.ipv4.tcp_synack_retries=5 1>/dev/nul

echo -e "\u2328 Disable IP-source routing"
sysctl -w net.ipv4.conf.all.accept_source_route=0 1>/dev/nul

echo -e "\u2328 Disable ICMP redirect"
sysctl -w -q net.ipv4.conf.all.accept_redirects=0 1>/dev/nul
sysctl -w net.ipv6.conf.all.accept_redirects=0 1>/dev/nul
sysctl -w net.ipv4.conf.default.accept_redirects=0 1>/dev/nul
sysctl -w net.ipv6.conf.default.accept_redirects=0 1>/dev/nul

echo -e "\u2328 Only accept ICMP redirects on trusted gateways"
sysctl -w net.ipv4.conf.all.secure_redirects=1 1>/dev/nul

echo -e "\u2328 Deactivating ICMP redirects"
sysctl -w net.ipv4.conf.all.send_redirects=0 1>/dev/nul
...
sysctl -w net.ipv4.conf.all.rp_filter=1 1>/dev/nul
